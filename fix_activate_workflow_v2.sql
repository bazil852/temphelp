-- Fix activate_workflow RPC to use correct ON CONFLICT constraint
-- The issue: ON CONFLICT (workflow_id) doesn't match any constraint
-- Solution: Use ON CONFLICT (token) or (workflow_id, node_id)
...
BEGIN
  ...
  IF webhook_node IS NOT NULL THEN
    ...
    -- Store webhook token mapping (use correct ON CONFLICT)
    INSERT INTO workflow_webhooks (workflow_id, node_id, token, created_at)
    VALUES (p_workflow, webhook_node.key, webhook_token, NOW())
