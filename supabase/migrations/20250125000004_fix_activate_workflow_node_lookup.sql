...
-- The issue: RPC was looking for wrong column names and wrong node structure
...
DECLARE
  ...
  sql_command text;
BEGIN
  -- Check if workflow exists and belongs to current user
  IF NOT EXISTS (
  ...
  -- Look for webhook trigger in exec_definition (converted format)
  SELECT n INTO webhook_node
  FROM workflows w,
       jsonb_each(COALESCE(w.exec_definition, '{}'::jsonb)) as n(key, value)
  WHERE w.id = p_workflow
    AND n.value->>'kind' = 'trigger' 
    AND n.value->>'sub' = 'webhook' 
  LIMIT 1;

  -- Look for schedule trigger in exec_definition (converted format)  
  SELECT n INTO schedule_node
  FROM workflows w,
       jsonb_each(COALESCE(w.exec_definition, '{}'::jsonb)) as n(key, value)
  WHERE w.id = p_workflow
    AND n.value->>'kind' = 'trigger' 
  ...
  IF webhook_node IS NOT NULL THEN
    ...
    webhook_url := 'https://your-project.supabase.co/functions/v1/wf-webhook/' || webhook_token;
