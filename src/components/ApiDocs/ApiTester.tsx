...
export default function ApiTester({ endpoint }: ApiTesterProps) {
  ...
  // Create initial request body function
  const createInitialRequestBody = () => {
    if (!endpoint || !endpoint.data.requestBody?.content?.['application/json']?.schema?.properties) {
      return '';
    }
    
    const properties = endpoint.data.requestBody.content['application/json'].schema.properties;
  ...
  // Use effect to initialize request body when endpoint changes
  useEffect(() => {
    setResponse(null);
    setError(null);
    
    if (endpoint && endpoint.data.requestBody) {
      setRequestBody(createInitialRequestBody());
    }
  }, [endpoint]);
  ...
  const handleTestEndpoint = () => {
    ...
    setTimeout(() => {
      if (data.responses) {
        ...
        const successCode = Object.keys(data.responses).find(code => code.startsWith('2'));
        
        if (successCode && data.responses[successCode]?.content?.['application/json']?.schema) {
          try {
            // Create a mock response based on the schema
            const mockResponse = createMockResponse(data.responses[successCode].content['application/json'].schema);
      ...
      <div className="mb-6">
        ...
        <div className="bg-gray-800 rounded-lg overflow-hidden mb-4">
          <div className="px-4 py-3 bg-gray-800 border-b border-gray-700 flex items-center">
            <span className={`inline-block w-16 rounded text-xs font-mono text-center py-1 ${getMethodColor(method)}`}>
              {method.toUpperCase()}
            </span>
            <span className="ml-2 text-sm text-gray-300 font-mono">{path}</span>
          </div>
          ...
          <div className="p-4">
            <div className="mb-4">
              ...
              <input
                type="text"
                className="w-full bg-gray-900 border border-gray-700 rounded-md py-2 px-3 text-sm text-white focus:outline-none focus:ring-1 focus:ring-green-500"
            ...
            {method.toUpperCase() !== 'GET' && method.toUpperCase() !== 'DELETE' && (
              <div className="mb-4">
                ...
                <div className="relative">
                  <textarea
                    className={`w-full bg-gray-900 border ${error ? 'border-red-500' : 'border-gray-700'} rounded-md py-2 px-3 text-sm text-white font-mono min-h-[200px] focus:outline-none focus:ring-1 focus:ring-green-500`}
                ...
                <div className="flex justify-between mt-2">
                  <button
                    ...
                    onClick={() => setRequestBody(createInitialRequestBody())}
                  >
                    Reset to Example
                  </button>
                  <button
        ...
        {response && (
          <div className="mt-6">
            ...
            <pre className="bg-gray-800 border border-gray-700 rounded-md p-4 text-sm text-green-300 font-mono overflow-x-auto">
              {response}
            </pre>
          </div>
        )}
